...
const LoanDetailsNew = props => {
 ...
 const [selectedIds, setSelectedIds] = useState([]);
  ...
  const fetchAppliedLoan = async () => {
    ...
    console.log('LoanDetailsNew.js line-45:', props?.route?.params?.loanId)
    const body = {
  ...
  const payment = () => {
    return (
      <View
        ...
        {/* https://instantmudra.com/admin/API/payWithRazorpay/104004/IM-PL-18128/4000 */}
        <View style={{ height: "30%", width: '80%', backgroundColor: 'white', justifyContent: "center", alignItems: "center", borderWidth: 2, borderRadius: 6, borderColor: '#B53059' }}>
          ...
          {/* <LinearButton
            onPress={() => {
              ...
              props.navigation.navigate('RazorpayPaymentPage', {
                user_id: data?.user_id,
                order_id: data?.order_id,
                emiAmount: data?.collection_amount
              })
            }}
            title={'Razorpay'} width={170} marginTop={20} /> */}
          <LinearButton
            onPress={() => {
              setShowPaymentPopup(false)
    ...
    <LinearGradient
      ...
      <Header navigation={props.navigation} title={'Loan Details'} />
       ...
       {three_month_emi ? ( 
        ...
        </ScrollView>  ):( <ScrollView style={{ flex: 1, backgroundColor: 'white', padding: 20, }}>
        ...
        <View
          ...
          <View style={styles.field}>
            ...
            <Text style={[styles.fieldText1, { color: '#596FB7' }]}>
              ...
              {data?.applied_date ? moment(data?.disbursed_date).format('DD-MM-YYYY') : moment(data?.date_created).format('DD-MM-YYYY')}
...
<View style={styles.field}>
            <Text style={styles.fieldText}>Loan Due Date</Text>
            <Text style={styles.fieldText1}>
              {/* // {moment(data?.repayment_date).format('DD-MM-YYYY') + '' ? moment(data?.repayment_date).format('DD-MM-YYYY') + '' : Null} */}
              {/* {moment(data?.repayment_date).format('DD-MM-YYYY') + ' ' ? Null : moment(data?.repayment_date).format('DD-MM-YYYY') + ' '} */}
              {moment(data?.repayment_date).format('DD-MM-YYYY')}
              {/* {moment(data?.repayment_date).format('DD-MM-YYYY') + '' ? moment(data?.repayment_date).format('DD-MM-YYYY') + '' : <Text>N/A</Text>} */}
